...
std::ostream &operator<<(std::ostream &os, const Dados &d){
    os << "Dado 1: " << d.d1_ << "\n" << "Dado 2: " << d.d2_ << "\n";
    return os;
}

std::istream &operator>>(std::istream &is, Dados &d){
    int aux;

    std::cout << "Introduzca un valor para el dado 1: ";
    std::cin >> aux;

    while(!d.setDado1(aux)){
        std::cout << "ERROR: Valor no válido. Introduzca un número entre 1 y 6: ";
        std::cin >> aux;
    }

    std::cout << "Introduzca un valor para el dado 2: ";
    std::cin >> aux;
